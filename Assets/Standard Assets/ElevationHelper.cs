...
public static class ElevationHelper
{
    ...
    static int GetNewVertex4(int i1, int i2)
    {
		...
		Vector3 tempVert = (vertices[i1] + vertices[i2]) / 2.0f;
//		if (vertices[i1].y <= vertices[i2].y) {
//			tempVert.y = vertices[i1].y;
//		}
//		if (vertices[i1].y > vertices[i2].y) {
//			tempVert.y = vertices[i2].y;
//		}
//		tempVert.y = Random.Range(vertices[i1].y, vertices[i2].y);
		var tempDist = Vector3.Distance(vertices[i1], vertices[i2])/20.0f;
		
		tempVert.y += Random.Range((0.0f-tempDist), tempDist);
    ...
    static int GetNewVertex9(int i1, int i2, int i3)
    {
		...
		Vector3 tempVert = (vertices[i1] + vertices[i2] + vertices[i3]) / 3.0f;
		if ((i1 != i2) && (i2 != i3)) {
			tempVert = (vertices[i1] + vertices[i2] + vertices[i3]) / 3.0f;
			if (Mathf.Approximately(vertices[i1].y, vertices[i2].y)) {
				tempVert.y = vertices[i1].y;
			}
			if (Mathf.Approximately(vertices[i2].y, vertices[i3].y)) {
				tempVert.y = vertices[i2].y;
			}
			if (Mathf.Approximately(vertices[i3].y, vertices[i1].y)) {
				tempVert.y = vertices[i3].y;
			}
		}
		if ((i1 == i2) && (i1 != i3)) {
			tempVert.y = vertices[i1].y;
